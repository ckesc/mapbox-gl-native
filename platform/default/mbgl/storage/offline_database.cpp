...
int OfflineDatabase::userVersion() {
    return getPragma<int64_t>("PRAGMA user_version");
}
...
mapbox::sqlite::Statement& OfflineDatabase::getStatement(const char* sql) {
    ...
    return statements
        .emplace(std::piecewise_construct, std::forward_as_tuple(sql),
                 std::forward_as_tuple(*db, sql))
        .first->second;
}
...
OfflineRegionMetadata OfflineDatabase::updateMetadata(const int64_t regionID, const OfflineRegionMetadata& metadata) {
    // clang-format off
    mapbox::sqlite::Query query{ Statement stmt = getStatement(
